...
本项目没有复用过往项目代码片段；业务代码、前端页面、Agent 转换流程、YAML Schema、示例小说和示例 YAML 均为本次作品原创实现。第三方库只作为基础框架、构建工具、数据库驱动、图标/动效组件和模型服务接入层使用。

后端 Maven 依赖：

| 依赖 | 用途 |
| --- | --- |
| Spring Boot Web / Validation / Security / Data JPA | REST API、参数校验、登录鉴权、数据库访问基础能力 |
| Spring AI OpenAI Starter | 接入 OpenAI-compatible ChatModel，支持真实模型转换与工具调用 |
| Spring AI RAG / PGVector Starter | 可选 RAG 检索增强和向量库接入，默认可关闭 |
| Flyway | 数据库迁移 |
| SnakeYAML | YAML 解析、生成结果校验和修复后的结构检查 |
| PostgreSQL Driver / H2 | 生产数据库连接与本地 dev profile 演示数据库 |
| Spring Boot Test / Spring Security Test | 后端单元测试和认证相关测试 |
...
| --- | --- |
| Vue 3 | 单页应用 UI 框架 |
| Vite / @vitejs/plugin-vue | 前端开发服务器和构建工具 |
...
| 依赖或服务 | 用途 |
| --- | --- |
| Docker / Docker Compose | 构建并编排前后端容器 |
| Nginx | 前端静态资源服务和 `/api/` 反向代理 |
| OpenAI-compatible API | 真实模型推理服务；未配置时可使用 `dev` fallback 演示输出 |
| DashScope Embedding | 可选，仅在启用 RAG/PGVector 向量检索时使用 |

原创功能边界：

- 小说章节解析、转换模式选择、fast 转换链路、react 自主工具调用回退策略、YAML Schema 校验与一次修复流程由本项目实现。
- 剧本 YAML Schema 的字段设计和设计原因见 [docs/screenplay-yaml-schema.md](docs/screenplay-yaml-schema.md)。
- `examples/` 中示例文本为原创演示材料，不包含未授权作品内容。
